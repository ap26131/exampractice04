...
public class MileageCalculatorNoConversion extends Application {
	...
	private TextField tfCapacity = new TextField();
	...
	public void start(Stage primaryStage) {
		...
		// Task 2: populate the combo box options using some of the already-created
		...
		tfDistance.setPromptText(defaultEntry);
		tfCapacity.setPromptText(defaultEntry);
		...
		//mainPane.add(new Label("Selection Type"), 0, 0);
		...
		cmb1.getSelectionModel().selectedItemProperty().addListener(e -> {
			changeLabels();
			calcMileage();
		});
	...
	private void calcMileage() {
		...
		double result = 0.0;
		if (cmb1.getSelectionModel().getSelectedItem().equals(cmb1.getItems().get(1))) {
			// liters / 100KM
